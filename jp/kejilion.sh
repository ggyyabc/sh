...
docker_ssh_migration() {
	...
	restore_docker() {
		...
		read -p "復元するバックアップ ディレクトリを入力してください:" BACKUP_DIR
		...
		for f in "$BACKUP_DIR"/backup_type_*; do
			...
			if grep -q "compose" "$f"; then
				...
				[[ -z "$original_path" ]] && read -p "元のパスが見つかりません。復元ディレクトリのパスを入力してください:" original_path
				...
				read -p "Compose プロジェクトの復元を確認します [$project_name] からパス [$original_path] ? (y/n): " confirm
	...
	migrate_docker() {
		...
		install jq
		read -p "移行するバックアップ ディレクトリを入力してください:" BACKUP_DIR
		[[ ! -d "$BACKUP_DIR" ]] && { echo -e "${RED}バックアップディレクトリが存在しません${NC}"; return; }

		read -p "ターゲットサーバーIP:" TARGET_IP
		read -p "ターゲットサーバーの SSH ユーザー名:" TARGET_USER
	...
	delete_backup() {
		...
		read -p "削除するバックアップ ディレクトリを入力してください:" BACKUP_DIR
		[[ ! -d "$BACKUP_DIR" ]] && { echo -e "${RED}バックアップディレクトリが存在しません${NC}"; return; }
		rm -rf "$BACKUP_DIR"
...
while true; do
	...
	97|wgs)
		...
		read -p "ネットワーク内のクライアントの数を入力してください (デフォルトは 5):" COUNT
		COUNT=${COUNT:-5}
		...
		for d in /config/peer_*; do
		  ...
		  echo "QRコードを生成します:$base_name.png"
...
echo "------------------------"
echo -e "${gl_kjlan}スクリプト公式サイト：${gl_bai}https://kejilion.sh            ${gl_kjlan}GitHub地址: ${gl_bai}https://github.com/kejilion/sh${gl_bai}"
echo "------------------------"
echo ""
}
