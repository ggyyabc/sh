...
docker_ssh_migration() {
	...
	backup_docker() {
		...
		read -p "백업할 컨테이너의 이름을 입력하십시오(여러 개의 공백을 구분하고 Enter를 눌러 실행 중인 모든 컨테이너를 백업하십시오)." containers
	...
	restore_docker() {
		...
		send_stats "도커 복원"
		read -p "복원할 백업 디렉터리를 입력하십시오:" BACKUP_DIR
		...
		for f in "$BACKUP_DIR"/backup_type_*; do
			...
			if grep -q "compose" "$f"; then
				...
				[[ -z "$original_path" ]] && read -p "원래 경로를 찾을 수 없습니다. 복원 디렉터리 경로를 입력하십시오:" original_path
				...
				read -p "Compose 프로젝트 복원 확인 [$project_name] 경로 [$original_path] ? (y/n): " confirm
				[[ "$confirm" != "y" ]] && read -p "새 복원 경로를 입력하십시오." original_path
	...
	migrate_docker() {
		...
		install jq
		read -p "마이그레이션할 백업 디렉터리를 입력하세요." BACKUP_DIR
		[[ ! -d "$BACKUP_DIR" ]] && { echo -e "${RED}백업 디렉터리가 존재하지 않습니다.${NC}"; return; }
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}21.  ${color21}VScode 웹 버전${gl_kjlan}22.  ${color22}UptimeKuma监控工具"
