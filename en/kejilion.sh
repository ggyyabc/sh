...
docker_ssh_migration() {
	...
	backup_docker() {
		...
		read -p "Please enter the name of the container to be backed up (separate multiple spaces and press Enter to back up all running containers):" containers
	...
	restore_docker() {
		...
		read -p "Please enter the backup directory to be restored:" BACKUP_DIR
		...
		for f in "$BACKUP_DIR"/backup_type_*; do
			...
			if grep -q "compose" "$f"; then
				...
				[[ -z "$original_path" ]] && read -p "Original path not found, please enter the restore directory path:" original_path
				...
				read -p "Confirm to restore Compose project [$project_name] to path [$original_path] ? (y/n): " confirm
	...
	migrate_docker() {
		...
		install jq
		read -p "Please enter the backup directory to be migrated:" BACKUP_DIR
		[[ ! -d "$BACKUP_DIR" ]] && { echo -e "${RED}The backup directory does not exist${NC}"; return; }
	...
	delete_backup() {
		...
		read -p "Please enter the backup directory to be deleted:" BACKUP_DIR
		[[ ! -d "$BACKUP_DIR" ]] && { echo -e "${RED}The backup directory does not exist${NC}"; return; }
		rm -rf "$BACKUP_DIR"
...
while true; do
	...
	97|wgs)
		...
		read -p "Please enter the number of clients in the network (default 5):" COUNT
		COUNT=${COUNT:-5}
		...
		for d in /config/peer_*; do
		  ...
		  echo "Generate QR code:$base_name.png"
